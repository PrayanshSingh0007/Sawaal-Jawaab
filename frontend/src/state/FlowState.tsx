...
import { listExchanges, listPhrases, saveExchange, savePhrase } from '../lib/db'
import { createHandoff, sendReply, watchHandoff } from '../lib/handoff'
 ...
 *
 * The draft question exists the moment the person types it. Everything after
 * that — variants, handoff, simplification — is an improvement layered on top,
 * never a gate in front.
 */
...
export function FlowProvider({ children }: { children: ReactNode }) {
  ...
  /* The question the Show screen renders. Raw text is the floor, so Show is
  ...
  /** Adopts a handoff a screen created for itself, so replies are watched. */
  ...
  const receiveReply = useCallback(
    async (text: string) => {
      ...
      // Show the words immediately; the plain version and action settle after.
  ...
  /* One exchange per question, written only once the question has actually been
     shown to someone — typing is not history — and updated in place when the
     reply lands. */
  useEffect(() => {
  ...
  const value = useMemo<FlowApi>(
    ...
    [
      draft, setRaw, setMethod, setSymbols, startFrom, clearDraft,
      variants, tone, question, markShown, handoff, openHandoff, registerHandoff,
      suggestion, reply, understood, understanding, receiveReply,
      exchanges, phrases, refresh, recordUse,
    ],
  )
